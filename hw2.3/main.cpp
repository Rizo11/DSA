...
bool intersect(const my_Segment &a, const my_Segment &b) {
    ...
    bool xOnY = a_.left.y == mi2*a_.left.x + b2;
    bool xOnY2 = a_.right.y == mi2*a_.right.x + b2;
    bool yOnX = b_.left.y == mi1*b_.left.x + b1;
    bool yOnX2 = b_.left.y == mi1*b_.right.x + b1;
...
//        }
        for (int j = i + 1; j < segments.size(); j++) {
            if (intersect(segments[i], segments[j])) {
                cout << "INTERSECTION" << endl;
                cout << segments[i].left.x << " " << segments[i].left.y << " " << segments[i].right.x << " " << segments[i].right.y << endl;
                cout << segments[j].left.x << " " << segments[j].left.y << " " << segments[j].right.x << " " << segments[j].right.y << endl;
                return 0;
            }
        }
