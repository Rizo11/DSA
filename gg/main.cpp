...
int isIntersect(Segment arr[], int n, Segment& s1, Segment& s2)
{
    ...
    for (int i=0; i<2*n; i++)
    {
        ...
        else
        {
            ...
            { string s=to_string(next->index+1)+" "+to_string(prev->index+1);
                string sq=to_string(prev->index+1)+" "+to_string(next->index+1);
                if (mp.count(s)==0&&mp.count(sq)==0&&doIntersect(arr[prev->index], arr[next->index])) {
                    ans++;
                    mp[s]++;
                    s1 = arr[prev->index];
                    s2 = arr[prev->index];
...
int main() {
    ...
    if (isIntersect(arr, n, res1, res2)) {
        cout << "INTERSECTION\n";
        cout << res1.left.x << " " << res1.left.y << " " << res1.right.x << " " << res1.right.y << endl;
        cout << res2.left.x << " " << res2.left.y << " " << res2.right.x << " " << res2.right.y << endl;
    } else {
